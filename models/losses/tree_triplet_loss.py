...
class TreeTripletLoss(nn.Module):
    ...
    def forward(self, feats, labels=None, max_triplet=200):
        ...
        labels = torch.nn.functional.interpolate(labels,
                                                 (feats.shape[2]), mode='nearest')
        labels = labels.squeeze(1).long()
        assert labels.shape[-1] == feats.shape[-1], '{} {}'.format(labels.shape, feats.shape)

        labels = labels.view(-1)
        feats = feats.permute(0, 2, 1) # ([1, 256, 6]) -->  ([1, 6, 256])      #(b, c, h,w) -->(b, h,w,c)
        feats = feats.contiguous().view(-1, feats.shape[-1]) #(Nn,256)
...
#             label_neg = labels[index_neg][:min_size]
        ...
        if class_count==0:
            ...
            return triplet_loss, torch.tensor([0]).cuda()
