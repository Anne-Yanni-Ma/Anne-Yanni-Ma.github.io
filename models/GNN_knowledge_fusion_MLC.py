...
class get_model(nn.Module):
    def __init__(self):
        ...
        self.node_classifer_MC = MLP(mlp=[512, 256, 256, 184])
    ...
    def forward(self, obj_codes, pred_codes): #obj_codes(Nn,C_konw) pred_codes (Ne, C)
        ...
        for i in range(self.assimilation):
            ...
            node_weight_MC, edge_weight = self.node_classifer_MC(node_embed), self.edge_classifer(edge_embed) # ([7, 160])  # ([42, 27])
            node_weight = node_weight_MC[:, :160]
            ...
            g.reset_graph()
        return node_weight, node_weight_MC, node_embed, edge_weight  # ([7, 160])  # ([42, 27])
...
class get_loss(nn.Module):
    ...
    def forward(self, obj_feature, node_output_mc, edge_output, gt_obj, gt_rel):
        #objgt_onehot = self.prepare_onehot_objgt(gt_obj)  #([7, 160])
        #obj_loss = self.focal_loss_obj(node_output, objgt_onehot)
        obj_loss = self.loss(obj_feature, node_output_mc, gt_obj, weight=self.obj_w)
