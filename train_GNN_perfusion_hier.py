...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='GNN_knowledge_fusion_hier', help='model name [default: pointnet2,GNN_knowledge_fusion]')
    ...
    #parser.add_argument('--MLC', type=bool, default=True, help='use multi-label classification or not')
...
def main(args):
    ...
    experiment_dir = experiment_dir.joinpath('GNN_SGCls_hier')
    ...
    shutil.copy(os.path.join(PROJECT_DIR, 'train_GNN_perfusion_hier.py'), str(experiment_dir))

    # initial pretrained node/edge embedder


    address_node = os.path.join(PROJECT_DIR, 'log/obj_classification_hier/2024-08-27_01-42')
    sys.path.append(address_node)
    MODEL_node = importlib.import_module('models.obj_classification.obj_classification_hier')
    ...
    criterion = MODEL.HierarchicalLoss(alpha=0.1, beta=1, gamma=2, obj_w=obj_w, pred_w=pred_w).cuda()   # cuda
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, data in bar:
            ...
            _, _,obj_codes = node_embedder(pc_mat_node)  #([7, 1024, 3]) #([7, 512])
            _, pred_codes = edge_embedder(pc_mat_edge)

            optimizer.zero_grad()
            network = network.train()

            node_output, hier_obj_output, edge_output = network(obj_codes, pred_codes)
            loss = criterion(hier_obj_output, edge_output, gt_obj, gt_rel)
        ...
        with torch.no_grad():
            ...
            for i, data in test_bar:
                ...
                _, _, obj_codes = node_embedder(pc_mat_node)
                _, pred_codes = edge_embedder(pc_mat_edge)
            ...
            log_string('Curr rel R@20: %f; ' % (curr_rel_R20) + 'R@50: %f; ' % (curr_rel_R50) + 'R@100: %f;' % (curr_rel_R100))
            log_string('Curr NGC rel R@20: %f; ' % (curr_rel_R20_ngc) + 'R@50: %f; ' % (curr_rel_R50_ngc) + 'R@100: %f;' % (curr_rel_R100_ngc))
            log_string('Curr mR rel R@20: %f; ' % (curr_rel_mR20) + 'R@50: %f; ' % (curr_rel_mR50) + 'R@100: %f;' % (curr_rel_mR100))
            log_string('--Eval-- Best rel: R@20: %f; ' % (best_rel_R20) + 'R@50: %f; ' % (best_rel_R50) + 'R@100: %f; ' % (best_rel_R100))
            log_string('--Eval-- Best NGC rel: R@20: %f; ' % (best_rel_R20_ngc) + 'R@50: %f; ' % (best_rel_R50_ngc) + 'R@100: %f; ' % (best_rel_R100_ngc))
            log_string('--Eval-- Best mR rel: R@20: %f; ' % (best_rel_mR20) + 'R@50: %f; ' % (best_rel_mR50) + 'R@100: %f; ' % (best_rel_mR100))
