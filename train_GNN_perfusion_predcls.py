...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='GNN_knowledge_fusion', help='model name [default: pointnet2,GNN_knowledge_fusion,IMP]')
...
def main(args):
    ...
    experiment_dir = experiment_dir.joinpath('GNN_PredCls')
    experiment_dir.mkdir(exist_ok=True)
    ...
    address_node = os.path.join(PROJECT_DIR, 'log/obj_classification/2023-02-15_17-44')
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, data in bar:
            ...
            node_knowledge = torch.Tensor(np.load('./data/meta_embedding/meta_embedding_node_hyp.npy')).cuda()
            ...
            node_output, edge_output = network(obj_codes, pred_codes) #IN: obj_codes (Nn,C_konw) pred_codes (Ne, C) ######
            loss = criterion(node_output, edge_output, gt_obj, gt_rel)  # node_output (Nn,160) edge_output (Ne, 27)  are onehot
            loss.backward()
            optimizer.step()
            ...
            train_rel_recall.calculate_recall(node_output_eval, edge_output_eval, gt_obj, gt_rel)  #(Nn,160) (Ne, 27)  gt_obj (Nn,)  gt_rel (Ne, 3)
        ...
        with torch.no_grad():
            ...
            for i, data in test_bar:
                ...
                node_output, edge_output = network(obj_codes, pred_codes) #IN: obj_codes (Nn,C_konw:512) pred_codes (Ne, C:512)    # output ([7, 160])  # ([42, 27])
                loss = criterion(node_output, edge_output, gt_obj, gt_rel)  # node_output (Nn,160) edge_output (Ne, 27)  are onehot
                ...
                test_rel_recall.calculate_recall(node_output_eval, edge_output_eval, gt_obj, gt_rel)#(Nn,160) (Ne, 27)  gt_obj (Nn,)  gt_rel (Ne, 3)
