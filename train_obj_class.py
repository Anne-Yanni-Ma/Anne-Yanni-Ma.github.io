...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='obj_classification_MLC', help='model name [default: pointnet2,GNN_knowledge_fusion]')
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, data in bar:
            ...
            if args.model == "obj_classification":
            ...
            elif args.model =="obj_classification_MLC":
                #if i==13:
                    #print("check")
                node_output, node_output_mc, obj_codes = network(pc_mat_node)
                loss = criterion(obj_codes, node_output_mc, gt_obj)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for i, data in test_bar:
                ...
                '''node_output, obj_codes = network(pc_mat_node) #IN: obj_codes (Nn,512) # output ([7, 160]) 
                loss = criterion(node_output,  gt_obj)  # node_output (Nn,160) '''
